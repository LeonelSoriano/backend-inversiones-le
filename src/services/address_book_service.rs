...
pub fn find_all(conn: DbConn) -> ResponseWithStatus {
    ResponseWithStatus {
        ...
        response: Response {
            ...
            data: Some(serde_json::to_value(Person::find_all(&conn)).unwrap()),
        },
    }
}
...
pub fn find_by_id(id: i32, conn: DbConn) -> ResponseWithStatus {
    ...
    if let Some(person) = option_person {
        ResponseWithStatus {
            ...
            response: Response {
                message: String::from(message_constants::MESSAGE_OK),
                data: Some(serde_json::to_value(person).unwrap()),
            },
        }
    } else {
        ResponseWithStatus {
...
pub fn query(query: String, conn: DbConn) -> ResponseWithStatus {
    ResponseWithStatus {
        ...
        response: Response {
            message: String::from(message_constants::MESSAGE_OK),
            data: Some(serde_json::to_value(Person::query(query, &conn)).unwrap()),
        },
    }
}
